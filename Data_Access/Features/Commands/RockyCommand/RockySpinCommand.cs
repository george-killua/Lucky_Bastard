...
{
    public class RockySpinCommand:IRequest<Response<ValueSpinRespone>>
    {
    ...
    public class RockySpinCommandHandler : IRequestHandler<RockySpinCommand, Response<ValueSpinRespone>>
    {
        ...
        public RockySpinCommandHandler(IHttpContextAccessor accessor, MyDBContext Contextt)
        {
            _accessor = accessor;
            Context = Contextt;
        ...
        public async Task<Response<ValueSpinRespone>> Handle(RockySpinCommand request, CancellationToken cancellationToken)
        {
            ...
            if(Amount>user.Balance) return new Response<ValueSpinRespone>(false, ("Dont have enough money")); ;
            user.Balance -= Amount;
            ...
            if (random.Next(0, 100) > tempPOL)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    int d = GameSetting.ReturnedSymbols;
                    if (i == 0)
                    {

                        symbol[i] = GameSetting.ReturnedSymbols;
            ...
            for (int i = 1; i < Linesused; i++)
            {
                    ...
                    symbol[tempresult[0]] == symbol[tempresult[4]] && symbol[tempresult[0]] == symbol[tempresult[3]])
                {
                    ...
                    if (RockysymbolsOne.Wild == true)
                    {
                        ...
                        returnedValue.WinningRows.Add(tempresult);
